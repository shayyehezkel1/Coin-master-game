...
## הרצה קבועה על מחשב Windows (בלי שרת בענן)

הדרך הזו מריצה את הבוט כ**שירות Windows** על המחשב שלך: עולה אוטומטית כשהמחשב נדלק,
וקם מחדש לבד אם קרס. אין עלות חודשית – אבל המחשב חייב להיות דלוק ומחובר לאינטרנט כל הזמן
(ראו הערה על מצב שינה בסוף).

1. **התקנת Node.js** – אם עוד אין: להוריד את גרסת ה-LTS מ-[nodejs.org](https://nodejs.org)
   ולהתקין (אישור ברירות המחדל מספיק).
2. **הורדת הקוד** – שכפול הריפו (`git clone ...`) או הורדת ה-ZIP מ-GitHub לתיקייה על המחשב,
   ומעבר בתוכה לתיקיית `telegram-assistant`.
3. **הגדרת משתני סביבה** – להעתיק את `.env.example` לקובץ בשם `.env` (באותה תיקייה) ולמלא:
   ```
   TELEGRAM_BOT_TOKEN=<הטוקן מ-BotFather>
   ANTHROPIC_API_KEY=<מפתח API של Claude, אופציונלי לצ'אט החכם>
   ```
4. **פתיחת שורת פקודה כמנהל (Administrator)** – חיפוש "PowerShell" או "cmd" בתפריט התחל,
   קליק ימני → "הפעל כמנהל מערכת".
5. בתוך תיקיית `telegram-assistant`, להריץ:
   ```powershell
   npm install
   npm install node-windows
   npm run service:install
   ```
   הפקודה האחרונה מתקינה ומפעילה שירות Windows בשם `TelegramPersonalAssistant`.
6. **בדיקה שהשירות רץ** – לפתוח `services.msc` (חיפוש בתפריט התחל) ולחפש
   `TelegramPersonalAssistant` – הסטטוס אמור להיות "פועל / Running".
7. **לוגים** – נכתבים לתיקייה `telegram-assistant\windows-service\daemon\` (קבצי `*.log` /
   `*.err.log`) – שימושי לדיבוג אם הבוט לא מגיב.
8. **הסרת השירות** (אם צריך) – מתוך שורת פקודה כמנהל, בתוך `telegram-assistant`:
   ```powershell
...
אם המחשב (בעיקר לפטופ) נכנס לשינה, השירות מושהה עד שהוא מתעורר, והתזכורות לא יישלחו בזמן.
כדי שהבוט יהיה זמין תמיד:
- הגדרות → מערכת → צליל/צריכת חשמל ("Power & battery") → "מסך וספה" – לקבוע ש"המחשב לא
  נכנס לשינה לעולם" כשהוא מחובר לחשמל.
- אם זה לפטופ – ב"מה קורה כשסוגרים את המכסה" לבחור "לא לעשות כלום" (כשמחובר לחשמל).

## פריסה ל-Render (חלופה בענן, בתשלום)
